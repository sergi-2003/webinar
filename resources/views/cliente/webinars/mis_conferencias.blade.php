...
<div class="row g-4">
    ...
    {{-- 🚫 Evita error si el webinar fue eliminado --}}
    @continue(!$webinar)
        ...
        <div class="webinar-card card border-0 shadow-sm h-100 overflow-hidden">
            <div class="webinar-header position-relative">
                <span class="badge estado {{ $estadoColor }}">
                    <i class="bi {{ $estadoIcon }} me-1"></i> {{ ucfirst(str_replace('_', ' ', $webinar->estado)) }}
                </span>
            ...
            <div class="card-body d-flex flex-column p-4">
                ...
                <div class="d-flex flex-column gap-2 mt-3">
                    ...
                    <button class="btn btn-outline-secondary rounded-pill fw-semibold shadow-sm w-100" disabled>
                        <i class="bi bi-clock-history me-1"></i> Disponible el
                        {{ $fechaWebinar->format('d/m/Y H:i') }}
                    </button>
                    @endif

                    <form action="{{ route('cliente.inscripcion.cancelar', $inscripcion->id) }}" method="POST"
                        class="cancel-form">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
                            class="btn btn-outline-danger rounded-pill fw-semibold shadow-sm w-100">
            ...
            <div class="card-footer bg-white border-0 text-center py-5">
